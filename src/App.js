...
function AComp() {
  ...
  // query string 을 얻는 방법 --> useSearchParams
  ...
  return (
    <Box>
      {customer && (
        <Text>
          {searchParams.get("id")} 번 고객명 {customer.cname}
        </Text>
      )}
    </Box>
  );
...
function BComp() {
  // dynamic param을 얻는 hook (useParams)
...
const routes = createBrowserRouter(
  createRoutesFromElements(
    <Route path="/" element={<Home />}>
      ...
      {/* dynamic param : spring web mvd 의 path variable 과 유사 */}
      {/* 경로의 일부분을 값으로 --> /: */}
      {/* useParams로 받는다 */}
